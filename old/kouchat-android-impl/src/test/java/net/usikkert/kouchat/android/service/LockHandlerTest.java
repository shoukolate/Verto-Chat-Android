...
import org.dynamicsoft.vertochat.android.settings.AndroidSetting;
...
public class LockHandlerTest {
    ...
    public void setUp() {
        ...
        when(powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "VertoChat wake lock")).thenReturn(wakeLock);
        when(wifiManager.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "VertoChat wifi lock")).thenReturn(wifiLock);
        when(wifiManager.createMulticastLock("VertoChat multicast lock")).thenReturn(multicastLock);
    ...
    public void constructorShouldCreateLocksAndRegisterListeners() {
        verify(powerManager).newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "VertoChat wake lock");
        verify(wifiManager).createWifiLock(anyInt(), anyString());
        verify(wifiManager).createMulticastLock("VertoChat multicast lock");
    ...
    public void constructorShouldCreateOlderWifiLockOnApi10() {
        verify(wifiManager).createWifiLock(WifiManager.WIFI_MODE_FULL, "VertoChat wifi lock");
    }
    ...
    public void constructorShouldCreateNewWifiLockOnApi12() {
        verify(wifiManager).createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "VertoChat wifi lock");
    }

...
public class AwayTest extends ActivityInstrumentationTestCase2<MainChatController> {
    ...
    public void setUp() {
        ...
        if (image == null) {
            FileUtils.copyVertoChatImageFromAssetsToSdCard(getInstrumentation(), activity);
            image = FileUtils.getVertoChatImageFromSdCardWithContentUri(activity);
        }
    }
    ...
    private void checkIfAway() {
        ...
        assertEquals(me.getNick() + " (Away) - VertoChat", supportActionBar.getTitle());
    }
    ...
    private void checkIfBack() {
        ...
        assertEquals(me.getNick() + " - VertoChat", supportActionBar.getTitle());
    }
